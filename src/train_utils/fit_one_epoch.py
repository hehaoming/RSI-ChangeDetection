...
def fit_one_epoch(module, train_data, test_data,args, optimizer=None, loss_dice=None, loss_bce=None,
                  is_cuda=False, val=None, epoch=None):
    val.reset()
    loss_set = torch.tensor([0], dtype=torch.float)
    module.train()
    print("start train Epoch: {}".format(epoch), end='\n')
    with tqdm(total=len(train_data), desc="Epoch{}".format(epoch), postfix=dict, mininterval=0.3) as pbar:
    ...
    if (epoch >0 and epoch % args.test_step==0):
        with tqdm(total=len(test_data), desc="Epoch{}".format(epoch), postfix=dict, mininterval=0.3) as pbar:
